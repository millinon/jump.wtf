    ...
    if($action === 'new'){
        ...
        if($type === 'file'){
            ...
            $result = $s3client->putObject(array(
				...
				error_log("Failed to upload to S3: " . $ex->getMessage() . " [" . $ex->getAwsErrorCode() . "]" );
			...
			if(jump_config::SAVE_BACKUP){
				...
				if($zip->open(jump_config::UBASEDIR . '/' . $new_key . '_arc.zip', ZipArchive::CREATE | ZipArchive::OVERWRITE) === TRUE){
					...
					try {
						...
						error_log("Failed to upload to Glacier: " . $ex->getMessage() . " [" . $ex->getAwsErrorCode() . "]" );
						err("Upload failed, please try again.");
					}
		...
		$result = $dyclient->putItem(array(
			...
			error_log("Failed to add item to table: " . $ex->getMessage() . " [" . $ex->getAwsErrorCode() . "]" );
		...
		if(strlen($key) > key_config::length){
			err('Invalid URL.');
		} else if(strlen($pass) > jump_config::MAX_PASS_LEN || strlen($pass) === 0){
			err('Invalid password.');
		}
		...
		try {
			...
			error_log("DynamoDB failed: " . $ex->getMessage() . " [" . $ex->getAwsErrorCode() . "]" );
			err("Incorrect key or password.");
		}
		...
		if($isFile){
			...
			try {
				...
				error_log("Failed to delete file from S3: " . $ex->getMessage() . " [" . $ex->getAwsErrorCode() . "]" );
			...
			if($isPrivate && aws_config::CF_DIST_ID !== ""){
				try {
					$cfclient->createInvalidation(array(
								...
								'CallerReference' => 'jumpwtf.delete-'. $filename . '.' . rand(0,8),
								'Paths' => array(
									'Quantity' => 1,
									'Items' => array('/' . $filename)
									)));
				} catch(CloudfrontException $ex){
					error_log("Failed to purge file from CF distribution: " . $ex->getMessage() . " [" . $ex->getAwsErrorCode() . "]" );
				}
			}
		}
