...
TCHAR *szTitle = _T("Jump.wtf");		// タイトル バーのテキスト
TCHAR *szWindowClass = _T("JUMPWTFWIN");	// メイン ウィンドウ クラス名
TCHAR *szWindowClassL = _T("JUMPWTFWINL");	// レイヤー ウィンドウ クラス名
HWND hLayerWnd;
...
BOOL				uploadFile(HWND hwnd, std::wstring fileName);
//std::string			getId();
	...
	int       nCmdShow)
{
	...
	if (2 == __argc)
	{
		...
		if (isPng(__targv[1])) {
			...
			GetTempFileName(tmpDir, _T("jump"), 0, tmpFile);
...
ATOM MyRegisterClass(HINSTANCE hInstance)
{
	...
	wc.hIcon = LoadIcon(hInstance, MAKEINTRESOURCE(JUMP_ICON));
...
LRESULT CALLBACK WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_LBUTTONUP:
	{
		...
		*/
		...
		//GetTempPath(MAX_PATH, tmpDir);
		GetTempFileName(dir, _T("jmp"), 0, file);

		if (savePNG(file, newBMP)) {

			// うｐ
			if (!uploadFile(hWnd, file)) {
				// アップロードに失敗...
				// エラーメッセージは既に表示されている
...
BOOL uploadFile(HWND hwnd, std::wstring fileName)
{
	...
	try {
		response = jump::get_upload_url(false);
	}
	catch (jump::JumpException je) {
		MessageBox(hwnd, _T("genUploadURL Failed "), szTitle, MB_ICONERROR | MB_OK);
		return FALSE;
	}
	...
	try {
		jump::upload_file(URL, std::string(fileName.begin(), fileName.end()), response["content-type"].asString());
	}
	catch (jump::JumpException je) {
		MessageBox(hwnd, _T("upload_file failed"), szTitle, MB_ICONERROR | MB_OK);
		return FALSE;
	}

	try {
		response = jump::gen_jump_file_url(response["tmp-key"].asString(), ".png");
	}
	catch (jump::JumpException je) {
		MessageBox(hwnd, _T("get_jump_file_url failed"), szTitle, MB_ICONERROR | MB_OK);
		return FALSE;
	}

	std::string newURL = response.isMember("cdn-url") ? response["cdn-url"].asString() : response["url"].asString();

	setClipBoardText(newURL.c_str());

	// URL を起動
	execUrl(newURL.c_str());
