...
function copy_button(string $id, string $label, string $href){
    return <button id={$id} class="btn btn-default copy-btn" data-clipboard-text={$href}>
...
function make_container(){
    ...
    if(isset($_SESSION['url'])){
        ...
        $container = <div class="container centered">
    ...
    $buttons = <div id="button-container" class="container centered"></div>;
        ...
        if($_SESSION['action'] === 'url_success' || $_SESSION['action'] === 'file_success'){
            $buttons->appendChild(copy_button('copy-url', 'Copy to clipboard', $_SESSION['url']));
            
            if(isset($_SESSION['cdn-url'])){
                $buttons->appendChild(<br/>);
                $buttons->appendChild(copy_button('copy-cdn', 'Copy direct link', $_SESSION['cdn-url']));
                $buttons->appendChild(<br/>);
            }
        }

        $buttons->appendChild(<div id="copy-success" class="alert alert-success copy-message">Copied to clipboard.</div>);
        $buttons->appendChild(<br/>);
        $buttons->appendChild(<div id="copy-failure" class="alert alert-warning copy-message">{"Couldn't copy to clipboard."}</div>); // this line messes up vim, so I wrapped it in {}s, sorry
...
function r_main(): void {
  ...
  if($_SESSION['action'] === 'url_success' || $_SESSION['action'] === 'file_success'){
    $body->appendChild(<script src={'//' . jump_config::CDN_HOST . '/js/' . file_get_contents('htdocs/js/clip.js.latest')}></script>);
  }
