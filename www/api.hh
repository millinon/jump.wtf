...
class jump_api {
    ...
    public static function jumpTo($input): ?array {
        ...
        if(isset($input['jump-key'])){
            ...
            $matches = [];
            if( !preg_match("~^/(" . key_config::regex . ")(\.[\w.]{1,".jump_config::MAX_EXT_LENGTH."})$", $toks['path'], $matches)){
                return self::error("Invalid URL specified", 400);        
            }
            $key = $matches[1];
        }

        $it = $dyclient->getIterator('Query',
            [
            ...
            foreach($it as $item){
                ...
                if($item['isFile']['N'] === 1){
                    if(isset($item['isPrivate'])){
                        ...
                        try {
                            $url = $s3client->getObjectUrl(aws_config::PRIV_BUCKET, $item['filename']['s'], '+15 minutes');  
        ...
        if(isset($expires)){
            return self::success(['url' => $url, 'is-file' => true, 'expires' => $expires->format(DateTime::ATOM)]);
        } else {
...
class apiHandler {
    ...
    private static function help($input): void {
        ...
        $options = ($_SERVER['REQUEST_METHOD'] === 'GET') ? (JSON_PRETTY_PRINT + JSON_UNESCAPED_SLASHES + JSON_UNESCAPED_UNICODE) : 0;

        switch($topic){

            case 'all':
                $out = json_encode(jump_api::$doc, $options);
            break;

            case 'constraints':
                $out = json_encode([
                    'description' => 'Specifies required parameters for a method',
                    'note' => 'Exactly one element of each array in the constraint array must be set',
                    'examples' => [
                        [
                            'constraint' => [ ['required'] ],
                        ...
                        [
                            'constraint' => [ ['option1', 'option2'], ['option2', 'option3']],
        ...
        default:
            $out = json_encode(jump_api::$doc[$topic], $options);
            break;
        }
