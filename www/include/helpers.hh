<?hh
    ...
    function in_tor(): bool {
        return isset($_SERVER['SERVER_NAME']) && preg_match('/\.onion$/', $_SERVER['SERVER_NAME']) == 1;
    }
    ...
    function filter_url(string $url): bool {
        ...
        if(!empty(jump_config::$banned_hosts) &&
            preg_match('/' . implode('|', jump_config::$banned_hosts) . '/i',
            $parsed['host']) == 1){
                return true;
            } else if(!empty(jump_config::$banned_terms) &&
            preg_match('/' . implode('|', jump_config::$banned_terms) . '/i', $url) == 1){
                return true;
            } else {
                return false;
            }
